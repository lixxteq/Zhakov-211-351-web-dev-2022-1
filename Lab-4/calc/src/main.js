...
window.onload = () => {
    ...
    const answerField = document.querySelector(".pre-answer");
    const buttons = document.querySelector(".buttons");

    const switchColors = (value) => {
        value === 'black' ? (() => {
            inputField.classList.remove('black');
            inputField.classList.add('grey');
    ...
    buttons.addEventListener('click', (e) => {
        let t = e.target;
        if (t.tagName === 'BUTTON') {
            if (t.textContent != 'C' && t.textContent != '=') {
