...
const showMessage = (msg, category = 'success') => {
    let div = document.createElement('div');
...
const findWinner = () => {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (diag != cells[i * 3 + i].innerHTML) diag = null;
        if (diag1 != cells[i * 3 + 2 - i].innerHTML) diag1 = null;
    }
    winner = diag || diag1;
    return winner;
};
...
const clickHandler = (event) => {
    ...
    event.target.innerHTML = count % 2 == 0 ? "&#215;" : "&#9675;";
    count++;
